...
library AIMStorage {
    ...
    struct Intent {
        ...
        bytes32 matchedWith;    // ID of the matched counterpart intent
    }
    ...
    /// @notice Decryption request tracking for async FHE operations
    struct DecryptRequest {
        euint128 encValue;     // The encrypted value awaiting decryption
        uint32 requestBlock;   // Block when decryption was requested
        bool resolved;         // Whether decryption has completed
        uint128 plainValue;    // The decrypted plaintext value (once resolved)
    }
    ...
    struct Storage {
        ...
        // Encrypted constants (initialized once, reused)
        euint128 ENC_ZERO;
        euint128 ENC_ONE;
        euint128 ENC_TOLERANCE;                       // matching tolerance (e.g., 5%)
        euint128 ENC_HUNDRED;
        // Decryption tracking
        mapping(bytes32 => DecryptRequest) decryptRequests;  // intentId -> decrypt state
    }
